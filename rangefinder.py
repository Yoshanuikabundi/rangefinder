...
print(
    ...
    "",
    "  Please cite:",
    "    Mitchell, J. A.; Whitfield, J. H.; Zhang, W. H.;",
    "    Henneberger, C.; Janovjak, H.; O'Mara, M. L.; Jackson,",
    "    C. J. 2016. Rangefinder: A semi-synthetic FRET sensor",
    "    design algorithm. ACS Sensors 1:1286-1290.",
...
try:
    ...
    python_version_number = '.'.join(str(i) for i in sys.version_info[0:3])
    print(
        'Rangefinder requires Python 3+,',
        'but was run with version %s. Exiting.' % python_version_number,
        sep=' ',
        file=sys.stderr
    )
    sys.exit(2)
...
try:
    ...
    print(
        'Rangefinder requires Biopython',
        '(http://biopython.org/wiki/Biopython),',
        'which cannot be found. Exiting.',
        sep=' ',
        file=sys.stderr
    )
    sys.exit(2)
...
class AppendDyesAction(argparse.Action):
    """argparse action to allow several dyes to be defined in a simple way

    Takes a single argument with one mandatory and 2 optional settings
    for each dye. Adapted from argparse.py"""
    ...
    def __init__(
        ...
        if nargs != '+':
            raise ValueError(
                'nargs must be * for AppendDyesAction to work as intended'
            )
        super().__init__(
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        if len(values) == 1:
            ...
            raise DyesArgumentError(
                "argument " + option_string + ": expected 1-3 arguments"
            )
        print(
            (
                "New dye \"{}\" defined with Förster distance {}"
                " and fudge factor {}."
            ).format(*out_tuple)
        )
...
parser = argparse.ArgumentParser(
    description="Predict dynamic range of sensors from crystal structures",
    add_help=False
)
...
required_named.add_argument(
    ...
    required=True,
    help=(
        "A comma-seperated list of ranges defining the residues by "
        "which to fit all structures."
    )
)
...
opt_args.add_argument(
    ...
    default='0',
    help=(
        "The residue number where the donor fluorophore is attached. "
        "0: use Rangefinder model of a fused fluorescent protein (default)."
    )
)
opt_args.add_argument(
    '-r',
    ...
    default='all',
    help=(
        "A comma-seperated list of residue index ranges defining the "
        "residues that will be considered for labelling with the "
        "acceptor fluorophore. Dynamic ranges will be predicted here."
    )
)
opt_args.add_argument(
    '-n',
    ...
    type=str,
    default='CA',
    help="Atomname to use for alignment and as the location of the terminus"
)
opt_args.add_argument(
    '-l',
    '--linker',
    type=float,
    default=20.0,
    help="Distance from first residue CA to fluorophore (Angstroms)"
)
opt_args.add_argument(
    '-d',
    '--dye',
    type=str,
    action=AppendDyesAction,
    metavar="PROPERTY",
    help=(
        "Define a different pair of fluorophores for your experiment. "
        "Takes up to 3 properties: the Förster distance of the interaction, "
        "a name for the fluorophore pair, and a fudge factor describing "
        "cross-talk. Omitted properties are to be similar to ECFP-AF532. "
        "May be specified multiple times for multiple dyes."
    )
)
opt_args.add_argument(
    '--advanced',
    action='store_true',
    help="Return predictions (rather than 0) for aligned residues."
)
opt_args.add_argument(
    '-v',
    '--verbose',
...
# Parse arguments, and if the new AppendDyesAction throws an error, produce
# a pretty error to match argparse
try:
    ...
    args_dict = {'prog': sys.argv[0], 'message': err}
...
if args.dye is None:
    global_dyes = [
        ("CFP-AF532", 4.80, 0.82),
        ("CFP-OG488", 4.80, 0.70),
        # More default dyes can be added here
    ]
...
def select_subset(
    ...
    """ Returns a list of atoms in the given entity matching the parameters

    For example, if residue is given as 47, a list of atoms in residue 47
    across all chains, models and structures will be returned"""
    ...
    # Base case: if we're at the end of the entity chain (ie, atoms),
    # return the current entity
    if type_of_entitys_children is None:
        return [entity]
    ...
    if restriction is None:
        ...
        restriction_correct_type = entity_selection_type[type(entity)](
            restriction
        )
        try:  # Try and select the desired child entity
            selected_children.append(entity[restriction_correct_type])
        except KeyError:
            # If the desired child isn't there, continue iterating over
            # the parent entities
            pass
...
def beads_from_model(residues, model, bead_name=atom_name):
    """Return a list of all beads with the specified name in the specified
    residue number range in model"""
    bead_dict = dict()
    for residue in model.get_residues():
        ...
        if residue_seq_id in residues:
            if residue_seq_id in bead_dict:
                raise Exception(
                    "Bead with residue sequence ID "
                    + str(residue_seq_id)
                    + " already in bead_dict: "
                    + str(residue)
                )
...
def get_all_interfluorophore_distances(
    fp_resnum,
    residues_to_check,
    residues_to_align,
    first_structures,
    ...
    """ Gets distances from all input structures based on alignment to
    all input structures in first_structures

    For consistency, I should align open to closed and closed to open and
    use fixed beads across everything. So I should align everything to a
    single open structure, get all 60 open distances, then redo that with
    a closed structure.

    Cool.

    That means this can be a single, simple function that I call twice
    with input lists concatenated in different orders. I will lose all
    the variation from movement in the sensing domain, but the whole
    point of this is that that's nonphysical.

    OK. Sounds like a PLAN!

    To avoid bias based on the single structure I align to, I repeat the
    above with every structure as the alignment target. Takes a bit longer
    but it works."""
    all_distances = {}
    longest_line_len = 0
    for structure in first_structures:
        # Make a list with all the structures except the current iteration
        other_first_structures = [
            x for x in first_structures if x != structure
        ]
        # Now we can put the current iteration at the front
        all_structures = (
            [structure] + other_first_structures + second_structures
        )

        # Align all models in structures (Side effect)
        aligned_target_model = align_structures(
            residues_to_align,
            all_structures,
            bead_name=atom_name,
            quiet=quiet
        ...
        for mutation in residues_to_check:
            selected_beads = select_subset(
                aligned_target_model,
                residue=mutation,
                atom=bead_name
            )
            ...
            if beads_found == 1:
                ...
                raise Exception(
                    "No bead found for mutation " + str(mutation)
                )
            else:
                raise Exception(
                    str(beads_found)
                    + " beads found for mutation "
                    + str(mutation)
                )
        ...
        for mutation, mutation_bead in fixed_mutation_beads.items():
            ...
            beads_to_measure = []
            # Iterate over the chains in structures, remembering where we
            # are at each step
            for structure in all_structures:
                # Throw up an error if we haven't been given a list of
                # structures
                if type(structure) != Bio.PDB.Structure.Structure:
                    raise TypeError(
                        "all_structures is not an iterable of structures!"
                    )
                num_expected_beads = len(structure)  # Should have 1 bead per model
                fluorophore_beads = select_subset(
                    structure,
                    residue=fp_resnum,
                    atom=bead_name
                )
                # Make sure the numbers of everything match up, just to
                # catch weird errors
                if len(fluorophore_beads) == num_expected_beads:
                    beads_to_measure = beads_to_measure + [
                        (fluor_bead, mutation_bead)
                        for fluor_bead in fluorophore_beads
                    ]
                else:
                    raise Exception(
                        "Bead count mismatch in structure " + str(structure)
                    )

            # Now we have a list of pairs of beads between which distances
            # must be measured, and we can just iterate over it!
...
def place_fluorophore(struct, dist, bead):
    ...
    single_fp_coord = pick_single_FPpoint(
        atoms,
        distance=dist,
        bead_name=bead
    )
    fp_atom = PDB.Atom.Atom(
        bead,
        single_fp_coord,
        0,
        1,
...
def load_distances_main(
    files_to_load_closed,
    files_to_load_open,
    fp_resnum,
    residues_to_check,
    residues_to_align,
    ...
    if residues_to_check == 'all':
        ...
        for struct in open_structures + closed_structures:
            for model in struct:
                # Store all the residues with alpha carbons from this model
                # in a set
                model_resis = set()
                for residue in model.get_residues():
                    if "CA" in residue:
    ...
    # Now, measure all the interfluorophore distances for the models we
    # just read
    open_distances = get_all_interfluorophore_distances(
        fp_resnum,
        residues_to_check,
        residues_to_align,
        open_structures,
    ...
    closed_distances = get_all_interfluorophore_distances(
        fp_resnum,
        residues_to_check,
        residues_to_align,
        closed_structures,
    ...
    distances_dict["Rangefinder predictions"] = {
        "open": open_distances,
        "closed": closed_distances
    }
...
def calculate_fret_efficiency(r, r_nought):
    """Calculate the FRET efficiency at r for a fluorophore pair with
    Forster distance r_nought."""
    r_nm = float(r / 10.0)
...
def dynamic_range(efficiency_bound, efficiency_apo, fudge):
    """Predict the dynamic range from two efficiencies and the overlap
    fudge factor."""
    try:
        dynamic_range = (
            (efficiency_bound - efficiency_apo)
            / ((1 - efficiency_bound) * (1 + (fudge * (efficiency_apo - 1))))
        )
...
def distances_to_efficiencies(distances_dict, dyes):
    """Converts a dict of distances to one of efficiencies given the
    dict of dyes

    distances_dict should be indexed by method, conformation and then
    mutation, with the final level being a list of distances, eg as
    produced by load_distances_main(). dyes should have r_nought as
    the second element (index 1). Output is indexed by dyes, method,
    mutation and then conformation to aid in dynamic range generation.
    Output is given as the mean, as efficiency is an ensemble average
    property
    """
        ...
        for method, conformations in distances_dict.items():
            ...
            for conformation, mutations in conformations.items():
                for mutation, distances in mutations.items():
                    # We want to select by mutation before conformation
                    # in the next step, which means changing things up
                    # here, but we don't want to lose objects if we've
                    # been here before
                    ...
                    efficiencies_list = [
                        calculate_fret_efficiency(r, r_nought)
                        for r in distances
                    ]
                    efficiencies[dye][method][mutation][conformation] = mean(
                        efficiencies_list
                    )
...
def efficiencies_to_dynamic_ranges(
    efficiencies,
    aligned_residues,
    advanced_mode
):
    """Converts a dict of efficiencies to one of dynamic ranges, averaged
    over the entire set of structures

    distances_dict should be indexed by dye (third element should be fudge
    factor), mutation and then conformation, with the final level being
    a list of efficiencies, eg as produced by distances_to_efficiencies()
    Output is indexed by conditions, which includes both dye and method,
    and then mutation, with each mutation's dynamic range"""
    ...
    for dye, methods in efficiencies.items():
        ...
        for method, mutations in methods.items():
            ...
            for mutation, conformations in mutations.items():
                ...
                # We need efficiencies from both conformations for this,
...
def dynamic_ranges_to_csv(in_dict, aligned_residues, filename):
    ...
    for condition, mutations in in_dict.items():
        for mutation, dynamic_range in mutations.items():
            ...
            # # We want a column that indicates the residues to which we
            # # aligned structures
            # aligned_title = "aligned?"
            # # only check aligned_residues once per mutation
            # if aligned_title not in organised_dict[mutation]:
...
def dict_to_csv(key_title, in_dict, filename):
    ...
    in_dict should be in format {key:{y_title[y]}}, with the title for
    the x values given as a parameter. Each dictionary in in_dict is thus
    a data line, printed as key,y1,y2,y3...

    Header line is taken from each y_dict and sorted, supports missing
    elements, and is presented as key_title,y_title1,y_title2,y_title3..."""
...
if __name__ == '__main__':
    ...
    global_distances_dict = load_distances_main(
        structures_to_load_closed,
    ...
    # Iterate over distances_dict and dyes to generate an analogous dictionary
    # of efficiencies
    global_efficiencies = distances_to_efficiencies(
        global_distances_dict,
        global_dyes
    )

    # Now we have efficiencies in a reasonable format we can get dynamic ranges
    global_dynamic_ranges = efficiencies_to_dynamic_ranges(
        global_efficiencies,
        residues_to_align,
        advanced
    )
